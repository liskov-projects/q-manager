...
export default function QueuesGridAlternate() {
  ...
  const getBackgroundColor = (queueLength) => {
    return queueLength >= 4 ? "bg-sky-400" :        // Friendly sky-blue for 4+ items
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {queues.map((queue) => (
        <div
          key={queue.id}
          className={`flex flex-col items-center justify-center h-24 w-24 border-8 border-black ${getBackgroundColor(queue.queueItems.length)}`}
        >
